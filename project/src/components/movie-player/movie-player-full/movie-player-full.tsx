...
const MoviePlayerFull = (movie: Film) => {
  ...
  return (
    <>
      <VideoPlayer ref={videoRef} {...{ movie, isPlaying, isMuted, isFullscreen, handleProgressUpdate }} />
      <ExitPlayerButton id={Number(movie.id)} />
      <PlayerControls>
        <PlayerControls isRow>
          ...
          <div className="player__time-value">{
            isPlaying && progress ? minutesToHoursAndMinutes(
              Number(progress.toFixed(DURATION_DECIMALS)), true
            ) : minutesToHoursAndMinutes(movie.runTime, true)
          }
          </div>
        </PlayerControls>
